...
namespace TableMasterApi.Tests.Controllers
{
    public class AuthControllerTests : IClassFixture<JwtConfigFixture>
    {
        ...
        public async Task Login_ShouldReturnOk_AndPersistRefreshToken_WhenCredentialsAreValid()
        {
            ...
            userDal.Setup(x => x.CreateSession(login.Email, login.Password, It.IsAny<string>(), It.IsAny<DateTime>()))
                .Callback<string, string, string, DateTime>((_, _, hashedToken, expiry) =>
                {
            ...
            userDal.Verify(x => x.CreateSession(login.Email, login.Password, It.IsAny<string>(), It.IsAny<DateTime>()), Times.Once);
        }

        [Fact]
        public async Task Login_ShouldReturnUnauthorizedWithGenericMessage_WhenPasswordIsInvalid()
        {
            var userDal = new Mock<IUserDAL>();
            var authDal = new Mock<IAuthDAL>();
            ...
            userDal.Setup(x => x.CreateSession(user.Email, "wrong-password", It.IsAny<string>(), It.IsAny<DateTime>()))
            ...
            error.Value.Should().Be("Identifiants invalides.");

            userDal.Verify(x => x.CreateSession(user.Email, "wrong-password", It.IsAny<string>(), It.IsAny<DateTime>()), Times.Once);
        }

        [Fact]
        public async Task Login_ShouldReturnSameUnauthorizedMessage_WhenUserDoesNotExist()
        {
            var userDal = new Mock<IUserDAL>();
            var authDal = new Mock<IAuthDAL>();
        ...
        public async Task Login_ShouldReturnBadRequest_WhenCredentialsAreBlank()
        {
            ...
            userDal.Verify(x => x.CreateSession(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<DateTime>()), Times.Never);
        }
        ...
        public async Task ResetPassword_ShouldUpdatePasswordAndInvalidateTokens_WhenResetTokenIsValid()
        {
            ...
            userDal.Setup(x => x.ResetPassword(hashedToken, "NewPassword!2")).ReturnsAsync(true);
            ...
            userDal.Verify(x => x.ResetPassword(hashedToken, "NewPassword!2"), Times.Once);
        }
        ...
        public async Task Refresh_ShouldRotateTokens_WhenRefreshTokenIsValid()
        {
            ...
            userDal.Setup(x => x.RotateRefreshToken(hashedInput, It.IsAny<string>(), It.IsAny<DateTime>()))
                .Callback<string, string, DateTime>((_, hash, __) => savedHash = hash)
                .ReturnsAsync(user);
            ...
            userDal.Verify(x => x.RotateRefreshToken(hashedInput, It.IsAny<string>(), It.IsAny<DateTime>()), Times.Once);
        }
        ...
        public async Task Refresh_ShouldReturnBadRequest_WhenRefreshTokenIsBlank()
        {
            ...
            result.Result.Should().BeOfType<BadRequestResult>();
            userDal.Verify(x => x.RotateRefreshToken(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<DateTime>()), Times.Never);
        }
